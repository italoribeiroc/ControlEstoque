...
void fm_gestaoEstoque::on_tabWidget_currentChanged(int index)
{
    if(index==1){
        ...
        if(query.exec()){
            ...
            ui->tw_ge_produtos->setColumnWidth(0,165);
            ui->tw_ge_produtos->setColumnWidth(1,260);
...
void fm_gestaoEstoque::on_btn_ge_filtrar_clicked()
{
    ...
    if(ui->txt_ge_filtrar->text()==""){
        ...
        if(ui->rb_ge_codigo->isChecked()){
            ...
            busca="select id_produto,produto from tb_produtos where produto like '%"+ui->txt_ge_filtrar->text()+"%' order by id_produto";
        }
    }
    int contlinhas=0;
    QSqlQuery query;
